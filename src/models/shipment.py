...
class Shipment(Base):
    ...
    * modification_timestamp (TIMESTAMP): The last modification time.
    ...
    payroll_price: Mapped[Decimal] = mapped_column(Numeric(10, 2), server_default="0")
    ...
    def validate_origin_weight(self, key, value):
        if isinstance(value, str):
            try:
                ...
                raise ValueError(f"Invalid origin_weight value: {value}") from e

        if not isinstance(value, Decimal):
            raise TypeError("origin_weight must be a Decimal or convertible to Decimal")
