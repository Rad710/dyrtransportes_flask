...
CREATE DATABASE IF NOT EXISTS dyr_ai CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS 'llm_readonly'@'%' IDENTIFIED BY '${QUERY_DB_PASSWORD}';
GRANT SELECT ON dyr_ai.* TO 'llm_readonly'@'%';
...
echo "shared MySQL: created databases dyr_ai (read-only) and dyr_demo (read/write)."
